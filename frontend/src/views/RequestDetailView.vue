<template>
  <div v-if="req">
    ...
    <!-- Override Modal (Pure Bootstrap) -->
    <div class="modal fade" id="overrideModal" tabindex="-1" aria-hidden="true" :class="{ show: showOverride }" :style="{ display: showOverride ? 'block' : 'none' }">
      <div class="modal-dialog modal-dialog-centered modal-sm">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">⚡ Override Status</h5>
            <button type="button" class="btn-close" @click="showOverride = false"></button>
          </div>
          <form @submit.prevent="doOverride">
            <div class="modal-body">
              <div class="alert alert-warning mb-3">
                ⚠️ Super Admin override — use with care.
              </div>
              <div class="mb-3">
                <label for="overrideStatus" class="form-label">New Status <span class="text-danger">*</span></label>
                <select id="overrideStatus" v-model="overrideForm.status" class="form-select" required>
            ...
            <div class="modal-footer">
              ...
              <button type="submit" class="btn btn-danger">Override</button>
            </div>
          </form>
        </div>
      </div>
